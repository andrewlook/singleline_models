...
class HParams():
    architecture = 'Sketch-Transformer'

    dataset_source: str = 'look'
    dataset_name: str = 'epoch20240104_furtherfiltered_trainval09'
...
class Trainer():
    ...
    def __init__(self,
                 ...
                 use_wandb=False,
                 wandb_project='sketch-transformer-pytorch',
        ...
        if self.use_wandb:
            wandb.watch((self.transformer), log="all", log_freq=10, log_graph=True)

        # store learning rate as state, so it can be modified by LR decay
        self.learning_rate = self.hp.lr
        self.optimizer = optim.Adam(self.transformer.parameters(), self.learning_rate)
